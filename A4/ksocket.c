...
int k_socket(int domain, int type, int protocol) {
    ...
    for (int j = 0; j < 256; j++) {
        ...
        if (j>0 && j < 256) SM[i].rwnd.wndw[j] = j-1;
        else SM[i].rwnd.wndw[j] = -1;
    }
    SM[i].swnd.size=SM[i].rwnd.size=10;
    SM[i].swnd.start_seq=SM[i].rwnd.start_seq=1;
    SM[i].send_buffer_sz=10;
    for (int j = 0; j < 10; j++) SM[i].recv_buffer_valid[j] = 0;
...
ssize_t k_recvfrom(int sockfd, void *buf, size_t len, int flags,
    ...
    if (sm->recv_buffer_valid[sm->recv_buffer_pointer]) {
        ...
        sm->rwnd.size++;
        int seq = -1;
        for (int i = 0; i < 256; i++) if (sm->rwnd.wndw[i] == sm->recv_buffer_pointer) seq = i;
        sm->rwnd.wndw[seq] = -1;
        sm->rwnd.wndw[(seq+10)%256] = sm->recv_buffer_pointer;
        ...
        sm->recv_buffer_pointer = (sm->recv_buffer_pointer + 1) % 10;
        V(sem_SM);
